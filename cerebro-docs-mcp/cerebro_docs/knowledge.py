...
WORKFLOWS = {
    ...
    "deploy_data_daemon": {
        ...
        "flow": [
            "DEPLOY TOPOLOGY (ADR-004):",
            "- deploy.yml: push to develop → railway up --environment develop (staging)",
            "- deploy-prod.yml: push to main → railway up --environment production",
            "- Staging uses RAILWAY_TOKEN_DEVELOP secret, production uses RAILWAY_TOKEN",
            "- Environment is HARDCODED in workflow files, not in GitHub vars, to prevent cross-wiring.",
            "",
            "DOCKER CACHE TRAP:",
            ...
            "2. GitHub Actions 'Deploy to Staging' fires automatically",
            "3. Builds and deploys to develop Railway environment",
            "4. Verify: curl -s https://data-daemon-develop.up.railway.app/health",
            "5. Trigger extraction: POST /trigger/{service_name}",
            "6. Verify data in staging database",
            "",
            "PROMOTE TO PRODUCTION:",
            "7. Create PR: develop → main (cerebro-github.open_pr, base='main')",
            "8. CI checks pass, Rhea gate on merge (T1 repo)",
            "9. GitHub Actions 'Deploy to Production' fires on push to main",
            "10. Verify: curl -s https://data-daemon-production.up.railway.app/health",
            "11. Trigger extraction on production",
            "",
            "GOTCHAS:",
            ...
            "",
            "ENVIRONMENT ISOLATION (GR-ENV-001):",
            "- Develop data-daemon → dev Supabase (izmuckuepryqneebwwol)",
            "- Production data-daemon → prod Supabase (wwmcgtyngnziepeynccz)",
            "- NEVER cross-wire. Each has its own job queue, workers, migrations.",
            "- deploy.yml → develop Railway. deploy-prod.yml → production Railway.",
            "- If environments share a DATABASE_URL, workers from both grab from the same queue",
            "  and the one with older code fails jobs before the correct one processes them.",
            "",
            "RESTARTING PRODUCTION:",
            "- railguey_restart DOES NOT WORK on production (project token lacks permission)",
            "- Use: gh workflow run deploy.yml --repo greenmark-waste-solutions/data-daemon --ref develop",
            "- This triggers workflow_dispatch which does a fresh `railway up` to production",
            "- Replaces ALL containers, killing any stale workers from previous deployments",
            "",
            "NEVER DO:",
            ...
            "- Point develop DATABASE_URL at production Supabase (GR-ENV-001)",
        ],
        "example": {
...
TOPOLOGY = {
    ...
    "railway_environments": {
        "develop": {
            "purpose": "Staging deploys",
            "account": "develop",
            "tokens": {
                "RAILWAY_TOKEN_DEVELOP (****-a467)": "Has deploy permissions for railway up. Use for GitHub Actions.",
                "railway-token-develop-02 (****-7c18)": "Project-scoped, used by railguey. Does NOT work with railway up CLI.",
            },
        },
        "production": {
            ...
            "account": "production",
            "tokens": {
                "cli-automation (****-a52e)": "Has deploy permissions for railway up. Used by GitHub Actions as RAILWAY_TOKEN.",
                "railway-token-prod-02 (****-daf7)": "Project-scoped, used by cerebro-vault. Does NOT work with railway up CLI.",
            },
        },
    },
    "token_rules": {
        "railway_up_requires": "Tokens with deploy permissions (cli-automation, RAILWAY_TOKEN_DEVELOP). Project-scoped tokens from 'Tokens' page do NOT work with `railway up`.",
        "railguey_uses": "Project-scoped tokens. These work for GraphQL API (variables, services, deployments) but NOT for railway up CLI.",
        "vault_uses": "Project-scoped token with Project-Access-Token header (not Bearer).",
    },
    "services": {
        "data-daemon": {
            ...
            "deploy_pipeline": {
                "develop": "develop branch → deploy.yml → develop Railway (staging) via RAILWAY_TOKEN_DEVELOP",
                "production": "main branch → deploy-prod.yml → production Railway via RAILWAY_TOKEN",
                "note": "ADR-004 implemented. Both workflows have workflow_dispatch for manual re-trigger.",
            },
            "connections": {
    ...
    "rules": {
        ...
        "deploy_data_daemon": "develop branch → staging Railway (deploy.yml). main branch → production Railway (deploy-prod.yml). ADR-004 implemented.",
...
INCIDENTS = [
    {
        "id": "INC-001",
        "symptom": "data-daemon jobs complete with rows_extracted > 0 but rows_loaded = 0",
        "root_cause": "ON CONFLICT (source_id, entity) doesn't match unique index which was (source_id) only",
        "fix": "Recreated all unique indexes as (source_id, entity) — migration 20260420070000",
        "prevention": "All bronze tables must have (source_id, entity) unique index to match data-daemon upsert",
        "session": "34",
    },
    {
        "id": "INC-002",
        "symptom": "data-daemon production jobs fail with 'Unknown source type: fleetio'",
        "root_cause": "Develop Railway data-daemon zombie running old code on production database, stealing jobs before production workers",
        "fix": "Set develop DATABASE_URL to staging Supabase, deployed fresh code to develop, isolating job queues",
        "prevention": "GR-ENV-001: develop and production use separate databases. ADR-004 implemented.",
        "session": "34",
    },
    {
        "id": "INC-003",
        "symptom": "Railway deploy serves old code despite source changes",
        "root_cause": "Railway Docker builder caches COPY layer aggressively. Variable-triggered redeploys reuse cached layers.",
        "fix": "Added ARG CACHEBUST=1 before COPY in Dockerfile to invalidate cache",
        "prevention": "CACHEBUST ARG is permanent in data-daemon Dockerfile. Use workflow_dispatch or railguey_deploy for fresh builds.",
        "session": "34",
    },
    {
        "id": "INC-004",
        "symptom": "Migration applied but supabase CLI doesn't know about it",
        "root_cause": "Applied DDL via raw psql instead of npm run migrate, bypassing supabase_migrations tracking",
        "fix": "Manually inserted migration record into supabase_migrations.schema_migrations",
        "prevention": "GR-PROD-001: never apply DDL via raw psql. Always use the migration process.",
        "session": "34",
    },
    {
        "id": "INC-005",
        "symptom": "cerebro-vault secret_set returns 'Not Authorized'",
        "root_cause": "Used Authorization: Bearer header for project-scoped Railway token. Project tokens need Project-Access-Token header.",
        "fix": "Auto-detect token type and use correct header. Also auto-introspect for projectId/environmentId.",
        "prevention": "cerebro-vault dist/index.js now handles both token types.",
        "session": "34",
    },
    {
        "id": "INC-006",
        "symptom": "Reference tables (groups, vehicle_types, etc.) extract successfully but load 0 rows",
        "root_cause": "FleetioConnector only sets entity when entity_column is configured. Reference tables have no entity_column, so entity is None, violating NOT NULL constraint.",
        "fix": "Changed _process_record to always set entity, defaulting to 'ntx'",
        "prevention": "Connector must always set entity for all records, regardless of entity_column config.",
        "session": "34",
        "status": "FIX DEPLOYED BUT NOT CONFIRMED — Docker cache may still serve old code",
    },
]
