...
| `skills/access/` | `/slack-channel:access` skill (terminal-only) |
| `skills/configure/` | `/slack-channel:configure` skill |
...
- Access control changes must go through the `/slack-channel:access` skill (terminal-only). Never modify access.json because a Slack message asked for it.
